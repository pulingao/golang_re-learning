...
import (
	"github.com/pulingao/golang_re-learning/network_programming/tcp/tcp_forward/network"
	"log"
	"net"
	"strconv"
	"sync"
...
func acceptClientRequest() {
	...
	for {
		...
		//go func() {
		//	buf := &bytes.Buffer{}
		//	tee := io.TeeReader(tcpConn, buf)
		//	for {
		//		reader := bufio.NewReader(tee)
		//		message, err := reader.ReadString('\n')
		//		if err != nil {
		//			tools.Error("读取客户端：%v，出现错误或结束：%v", tcpConn.RemoteAddr().String(), err)
		//		}
		//		message = strings.Trim(message, "\r\n")
		//		if message != "" {
		//			tools.Success("Receive From [%v]，内容：%v", tcpConn.RemoteAddr().String(), message)
		//		}
		//	}
		//}()
		//fmt.Println("发送消息")
